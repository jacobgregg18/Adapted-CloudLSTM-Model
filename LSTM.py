...
model = CloudLSTMNextScan(in_dim=9, hidden_dim=384, msg_dim=96, k=0, T=T).to(device, dtype=torch.float32) # Ensure model parameters are float32
...
for epoch in range(num_epochs):
    model.train()
    total_loss = 0.0
    total_empirical_bias = 0.0
    total_pred_change = 0.0
    total_target_change = 0.0
    total_l1_loss = 0.0
    total_cloud_loss = total_cloud_loss1 = 0.0
    total_bg_loss = total_bg_loss1 = 0.0
    pct_active = 0.0
    total_l1_per = 0.0
    total_mse_loss = 0.0
    total_mse_per = 0.0
    ...
    for batch in train_loader:
        ...
        with torch.no_grad():
            ...
            # (Option A: raw shifted range [-20,20])
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            if len(point_clouds) < 7:
                continue
            
            # Prepare inputs
            input_pcs = point_clouds[:6]
            x = torch.stack(input_pcs, dim=1)
            
            pc_current = point_clouds[5]
            pc_target = point_clouds[6]
