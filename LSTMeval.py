...
start_epoch = 0
checkpoint_path = "Test16_LSTM_Epoch9.pt"
...
criterion = ImprovedCloudDynamicsLoss(
        ...
        huber_delta=0.2,        # Tune this: 0.15-0.25
        cloud_boost=1.5,        # Moderate boost (not 3.0!)
...
best = {
    'easy': {'model_mse': [10], 'model_mae': [10], 'input_number': [0]},
    'medium': {'model_mse': [10], 'model_mae': [10], 'input_number': [0]},
    'hard': {'model_mse': [10], 'model_mae': [10], 'input_number': [0]},
    'dynamic_region': {'model_mse': [10], 'model_mae': [10], 'input_number': [0]}
}
...
with torch.no_grad():
    for batch in val_loader:
        ...
        if model_mse < best[category]['model_mse'][0]:
            best[category]['model_mse'][0] = model_mse
            best[category]['model_mae'][0] = model_mae
            best[category]['input_number'][0] = number
        ...
        if dynamic_mask.any():
            ...
            if model_mse_dyn < best[category]['model_mse'][0]:
                best['dynamic_region']['model_mse'][0] = model_mse_dyn
                best['dynamic_region']['model_mae'][0] = model_mae_dyn
                best['dynamic_region']['input_number'][0] = number
...
print(f"  Easy  - MSE: {best['easy']['model_mse'][0]:.6f} | MAE: {best['easy']['model_mae'][0]:.6f} | Number: {best['easy']['input_number'][0]}%")
print(val_seqs[best['easy']['input_number'][0]])
print(f"  Medium  - MSE: {best['medium']['model_mse'][0]:.6f} | MAE: {best['medium']['model_mae'][0]:.6f} | Number: {best['medium']['input_number'][0]}%")
print(val_seqs[best['medium']['input_number'][0]])
print(f"  Hard  - MSE: {best['hard']['model_mse'][0]:.6f} | MAE: {best['hard']['model_mae'][0]:.6f} | Number: {best['hard']['input_number'][0]}%")
print(val_seqs[best['hard']['input_number'][0]])
print(f"  Dynamic  - MSE: {best['dynamic_region']['model_mse'][0]:.6f} | MAE: {best['dynamic_region']['model_mae'][0]:.6f} | Number: {best['dynamic_region']['input_number'][0]}%")
print(val_seqs[best['dynamic_region']['input_number'][0]])

   
for cat in ['easy', 'medium', 'hard', 'dynamic_region']:
    if len(results[cat]['model_mse']) == 0:
        continue
    
    persist_mse = np.array(results[cat]['persist_mse'])
    model_mse   = np.array(results[cat]['model_mse'])
    persist_mae = np.array(results[cat]['persist_mae'])
    model_mae   = np.array(results[cat]['model_mae'])

    mse_improvement = 100 * (persist_mse - model_mse) / persist_mse
    mae_improvement = 100 * (persist_mae - model_mae) / persist_mae
            
    best_mse = np.max(mse_improvement)
    index = np.argmax(mse_improvement)
    
    print(f"\n{cat.upper()} SCENES (n={len(results[cat]['model_mse'])}, corr={avg_corr:.3f}):")
    print(f"Best Improvement MSE: {best_mse}, index: {index}, datetime: {val_seqs[index]}")